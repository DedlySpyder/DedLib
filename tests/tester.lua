...
return function()
    ...
    if test_assert_counts["failed"] > 0 then
        error("Tester.assert_equals() tests are failing, cannot accurately run other tests at this time. See debug logs for more details.")
    end
    ...
    local function mockValidEntityTester(values, expectedEntity)
        ...
        Tester.assert_equals(expectedEntity, mock, "Mock valid entity failed: " .. serpent.line(values))
    end
    ...
    local addValidationForBasicAddTest = function(validationName, testerName, testName, expectedResult, errorMessage)
        test_validations[validationName] = function()
            ...
            Tester.assert_equals(testName, testResult.name, "Test <name> assert, failed")
            Tester.assert_equals(expectedResult, testResult.result, "Test <result> assert, failed")
            Tester.assert_starts_with("__DedLib__/tests/tester.lua:", testResult.test_location, "Test <result> assert, failed")
            Tester.assert_contains_exact(
                    testResult,
            ...
            if not expectedResult then
                Tester.assert_ends_with(errorMessage, testResult.error, "Test <error> assert, failed")
            end
        end
    end
    ...
    test_validations["add_tests_no_tests_tester_missing"] = function()
        ...
        Tester.assert_equals(nil, test_results["results"]["testers"][testerName], "Test missing test, found")
    end
    ...
    test_validations["add_tests_one_test_tester_missing"] = function()
        ...
        Tester.assert_equals(nil, test_results["results"]["testers"][testerName][testName], "Test missing test, found")
    end
